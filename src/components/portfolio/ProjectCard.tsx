...
const ProjectImage = memo(
  ({ icon, children }: { icon: string; children?: React.ReactNode }) => (
    <div className="relative h-40 sm:h-48 bg-gradient-to-br from-kick-green/10 to-kick-green-dark/10 dark:from-kick-green/20 dark:to-kick-green-dark/20 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-kick-green/20 to-kick-green-dark/20" />
      <div className="absolute top-3 right-3 sm:top-4 sm:right-4 text-3xl sm:text-4xl opacity-60 group-hover:opacity-100 transition-opacity duration-300">
        {icon}
      </div>
      {children}
    </div>
...
const DialogProjectContent = memo(
  ({
    ...
    <>
      <DialogHeader>
        <DialogTitle className="text-2xl sm:text-3xl flex items-center gap-2 sm:gap-3">
          <span className="text-3xl sm:text-4xl">{icon}</span>
          {title}
        </DialogTitle>
        <DialogDescription className="pt-3 sm:pt-4 text-base sm:text-lg leading-relaxed">
          {description}
        </DialogDescription>
      </DialogHeader>

      <div className="space-y-4 sm:space-y-6">
        <div className="relative h-48 sm:h-64 bg-gradient-to-br from-kick-green/10 to-kick-green-dark/10 dark:from-kick-green/20 dark:to-kick-green-dark/20 rounded-lg overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-kick-green/20 to-kick-green-dark/20" />
          <div className="absolute top-3 right-3 sm:top-4 sm:right-4 text-4xl sm:text-6xl opacity-60">
            {icon}
          </div>
        </div>

        <div>
          <h3 className="font-semibold text-base sm:text-lg mb-2 sm:mb-3 text-slate-700 dark:text-slate-300">
            Technologies Used
          </h3>
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <Badge key={tag} className="skill-badge px-2 sm:px-3 py-1 text-xs sm:text-sm">
                {tag}
              </Badge>
            ))}
          </div>
  ...
  ({
    ...
    return (
      <Dialog>
          ...
          >
            <Card className="h-full project-card glass-morphism hover-neon-subtle transition-all duration-500 overflow-hidden">
              <ProjectImage icon={icon}>
                <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 flex space-x-2">
                  <AnimatedButton
                    href={liveUrl}
                    icon={ExternalLink}
                    delay={0.2 + index * 0.1}
              ...
              <CardHeader className="pb-2 sm:pb-3">
                <CardTitle className="flex items-center gap-2 sm:gap-3 text-lg sm:text-xl group-hover:text-kick-green dark:group-hover:text-kick-green-light transition-colors duration-300">
              ...
              <CardContent className="pt-0">
                <p className="text-slate-600 dark:text-slate-300 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-3 leading-relaxed">
                  {description}
                </p>
                ...
                <div className="flex items-center justify-between">
                  <motion.div
                    whileHover={{ x: 5 }}
                    className="flex items-center text-xs sm:text-sm text-kick-green dark:text-kick-green-light font-medium"
                  >
        ...
        <DialogContent className="max-w-3xl glass-morphism mx-4 sm:mx-0">
